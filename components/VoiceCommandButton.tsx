...
interface VoiceCommandButtonProps {
  ...
  inline?: boolean; // quando true, renderiza como botão compacto no header
}
...
export const VoiceCommandButton: React.FC<VoiceCommandButtonProps> = ({ context, inline = false }) => {
  ...
  return (
    <>
      {/* ── Toasts (sempre visíveis independente do modo) ─────────────────── */}
      <div
        ...
        >
          <div className="bg-white dark:bg-dark-card shadow-xl rounded-2xl px-5 py-3 flex items-center gap-3 border border-mint/30 max-w-sm w-full">
            <span className={`w-3 h-3 rounded-full flex-shrink-0 ${
              isListening ? 'bg-red-500 animate-pulse' : 'bg-mint-dark'
            }`} />
      ...
      {/* ── Painel de ajuda ─────────────────────────────────────────────── */}
      {showHelp && (
        <div className="fixed right-4 top-24 z-50 bg-white dark:bg-dark-card rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 p-4 w-72 text-sm">
          <div className="flex justify-between items-center mb-3">
            <p className="font-bold text-dark-gray dark:text-white">Comandos de Voz</p>
            <button onClick={() => setShowHelp(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-lg leading-none">✕</button>
          </div>
          {!supported && (
          ...
          <div className="space-y-2 text-gray-600 dark:text-gray-400">
            {[
              ...
              <div key={cmd} className="flex gap-2 items-start">
                <code className="bg-gray-100 dark:bg-gray-700 rounded px-1.5 py-0.5 text-xs text-mint-dark font-mono flex-shrink-0 mt-0.5">{cmd}</code>
      ...
      {/* ── Modo inline: botão compacto no header ──────────────────────── */}
      {inline ? (
        <div className="flex items-center gap-1 flex-shrink-0">
          {/* Botão ? ajuda */}
          <button
            onClick={() => setShowHelp(v => !v)}
            className="w-8 h-8 rounded-full bg-mint-dark dark:bg-mint text-white text-xs font-bold hover:opacity-80 transition-opacity flex items-center justify-center shadow-sm"
          ...
          <button
            ...
            className={`relative w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 active:scale-95 shadow-md ${
              isListening  ? 'bg-red-500 scale-110' :
              isProcessing ? 'bg-mint-dark opacity-70' :
              supported    ? 'bg-mint-dark dark:bg-mint hover:opacity-80' : 'bg-gray-400'
            }`}
          >
            {isListening && <span className="absolute inset-0 rounded-full bg-red-400 animate-ping opacity-40" />}
            {isProcessing ? (
              <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
            ) : isListening ? (
              <svg className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2" /></svg>
            ) : (
      ...
      /* ── Modo flutuante (legado — não mais usado) ───────────────────── */
      <div className="fixed right-4 z-40 flex flex-col items-end gap-2" style={{ bottom: 'calc(1.5rem + env(safe-area-inset-bottom))' }}>
        {!isActive && (
          <button
            onClick={() => setShowHelp(v => !v)}
